...
def generate_neighbourhood(stop_list, tabu_list):
    ...
    for i in range(len(stop_list)):
        for j in range(i + 1, len(stop_list)):
            ...
            new_stop_list[i], new_stop_list[j] = new_stop_list[j], new_stop_list[i]  # Swap two stops
            
            # Convert to tuple for reliable comparison
...
def calculate_cost(graph, starting_stop, stop_list, start_time, optimalization_criteria):
    stop_list = [starting_stop] + stop_list + [starting_stop]  # Include the start and end stop
    total_cost = 0
    total_path = []
    current_start_time = start_time

    for i in range(len(stop_list) - 1):
        path, total_travel_time, route_cost = find_a_star_path(graph, stop_list[i], stop_list[i + 1], current_start_time, 'manhattan')
...
def tabu_search(graph, starting_stop, stop_list, start_time, optimalization_criteria):
    ...
    while len(tabu_list) < max_tabu_size and iteration < max_iterations:
        ...
        for new_solution in neighbourhood:
            ...
            costs[tuple(new_solution)] = [total_cost, total_path]  # Store as tuple
        ...
        # Add only the best neighbour to tabu list
        tabu_list.append(best_neighbour)
        if len(tabu_list) > max_tabu_size:
            tabu_list.pop(0)  # Keep tabu list size manageable
